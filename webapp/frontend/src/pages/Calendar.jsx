...
const Calendar = () => {
  ...
  const parseSingleFile = async (fileItem) => {
    try {
      ...
      const cleanDateStr = (raw) => {
        ...
        return /^\d{4}-\d{2}-\d{2}$/.test(dOnly) ? trimmed : '';
      };
      ...
      if (res.ok && result.success && result.extracted) {
        sDate = cleanDateStr(result.extracted.start_date);
        eDate = cleanDateStr(result.extracted.end_date);
        cpa = result.extracted.cpa_name || '';
        issueDate = cleanDateStr(result.extracted.issue_date) || '';
        ...
        const nextQueue = prev.map((item) => {
          if (item.id === fileItem.id) {
            return {
              ...
              cpa_name: cpa, // Strict: ONLY from this specific PDF file
    ...
    if (fileQueue.length > 0) {
      ...
      try {
        for (let i = 0; i < fileQueue.length; i++) {
          ...
          payload.append('end_date', item.end_date);
  ...
  const certYears = certificationsList.flatMap((c) => {
    if (!c) return [];
    const s = c.start_date ? new Date(c.start_date.includes('T') ? c.start_date : c.start_date + 'T00:00:00').getFullYear() : null;
    const e = c.end_date ? new Date(c.end_date.includes('T') ? c.end_date : c.end_date + 'T00:00:00').getFullYear() : null;
    return [s, e];
  }).filter((y) => y && !isNaN(y));
  ...
  const getCertsForMonth = (monthIndex) => {
    ...
    return certificationsList.filter((cert) => {
      if (!cert || !cert.start_date || !cert.end_date) return false;
      const cStart = new Date(cert.start_date.includes('T') ? cert.start_date : cert.start_date + 'T00:00:00');
      const cEnd = new Date(cert.end_date.includes('T') ? cert.end_date : cert.end_date + 'T23:59:59');
  ...
  // Compute active certification brackets spanning months in the selected year
  const getYearCertificationBrackets = () => {
    ...
    return certificationsList
      .map((cert) => {
        if (!cert || !cert.start_date || !cert.end_date) return null;
        const cStart = new Date(cert.start_date.includes('T') ? cert.start_date : cert.start_date + 'T00:00:00');
        const cEnd = new Date(cert.end_date.includes('T') ? cert.end_date : cert.end_date + 'T23:59:59');
        if (isNaN(cStart.getTime()) || isNaN(cEnd.getTime())) return null;
    ...
    <div style={{ padding: '2rem 2.5rem', maxWidth: '1400px', margin: '0 auto', color: 'var(--text-primary)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ fontSize: '1.8rem', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '0.75rem', margin: 0 }}>
            ...
            Calendario de Certificaciones Contables
          </h1>
          <p style={{ color: 'var(--text-secondary)', margin: '0.4rem 0 0 0', fontSize: '0.95rem' }}>
            Control de períodos auditados, validez temporal y dictámenes de contador público
          </p>
        </div>
      ...
      <div className="glass-card" style={{ padding: '1.75rem', borderRadius: '1rem', marginBottom: '2rem', position: 'relative', zIndex: 20, overflow: 'visible' }}>
        ...
        {activeBrackets.length > 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '0.6rem', marginBottom: '0.75rem' }}>
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', minWidth: 0 }}>
                    ...
                    <span style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>{b.cert.title}</span>
                  </div>
                  {b.cert.cpa_name && (
                  ...
                  <tr key={cert.id}>
                    ...
                    <td>
                      <span className="badge badge-blue">
                        {cert.start_date} al {cert.end_date}
                      </span>
                    </td>
